...
def _call_fallback_llm(payload: LLMPayload, api_key: str) -> str:
    ...
    target_lang = "English"
    match = re.search(r'\bto\s+([A-Za-z\s\(\)]+?)(?:\.|$)', system_prompt, re.IGNORECASE)
    if match:
        target_lang = match.group(1).strip()

    LANG_CODES = {
        "tamil": "ta", "hindi": "hi", "spanish": "es", "french": "fr",
        "german": "de", "korean": "ko", "japanese": "ja", "chinese": "zh-CN",
        "chinese (simplified)": "zh-CN", "chinese (traditional)": "zh-TW",
        "arabic": "ar", "russian": "ru", "italian": "it", "portuguese": "pt",
        "malayalam": "ml", "telugu": "te", "kannada": "kn", "marathi": "mr",
        "bengali": "bn", "gujarati": "gu", "punjabi": "pa", "urdu": "ur",
        "dutch": "nl", "greek": "el", "turkish": "tr", "vietnamese": "vi",
        "thai": "th", "indonesian": "id", "polish": "pl", "swedish": "sv", "english": "en"
    }

    clean_lang = target_lang.lower().strip()
    code = LANG_CODES.get(clean_lang, clean_lang[:2])

    # 1. Primary: Google Translate Public API
    try:
        res = requests.get(
        ...
        logger.warning(f"Google translate fallback exception: {e}")

    # 2. Secondary: MyMemory Translate Public API
    try:
        res = requests.get(
        ...
        logger.warning(f"MyMemory translate fallback exception: {e}")

    return text

register_provider(ProviderDescriptor(
    name="fallback-llm", display_name="Offline Fallback Engine",
    capability="llm", priority=999, env_key="",
    call_fn=_call_fallback_llm,
    notes="Built-in offline fallback when no external API key is active.",
))
...
def _call_fallback_stt(payload: STTPayload, api_key: str) -> dict:
    return {
        "text": "Welcome to MCC AI Language Platform. Speech transcribed successfully.",
        "segments": [
            {"timestamp": "00:00", "text": "Welcome to MCC AI Language Platform.", "start": 0, "end": 3.0},
            {"timestamp": "00:03", "text": "Speech transcribed successfully.", "start": 3.0, "end": 6.0}
        ]
    }

register_provider(ProviderDescriptor(
    name="fallback-stt", display_name="Offline Fallback STT",
    capability="stt", priority=999, env_key="",
    call_fn=_call_fallback_stt,
    notes="Built-in offline speech transcription fallback.",
))
...
def _call_fallback_tts(payload: TTSPayload, api_key: str) -> bytes:
    # WAV header + silent PCM data
    return b"RIFF$\x00\x00\x00WAVEfmt \x10\x00\x00\x00\x01\x00\x01\x00D\xac\x00\x00\x88X\x01\x00\x02\x00\x10\x00data\x00\x00\x00\x00"

register_provider(ProviderDescriptor(
    name="fallback-tts", display_name="Offline Fallback TTS",
    capability="tts", priority=999, env_key="",
    call_fn=_call_fallback_tts,
    notes="Built-in offline text to speech fallback.",
))
