...
def google_login(data: GoogleLoginRequest, db: Session = Depends(get_db)):
    try:
        ...
        traceback.print_exc()
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Google Login Error: {str(e)}")
...
def google_register_tenant(data: GoogleTenantRegistration, db: Session = Depends(get_db)):
    try:
        ...
        admin_name = idinfo.get("name") or (admin_email.split("@")[0].capitalize() if admin_email else "User")
        ...
        traceback.print_exc()
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Google Registration Error: {str(e)}")
