...
CHUNK_SIZE = 12000  # characters per chunk — fast processing for large documents
...
def translate_text(
    ...
    def _translate_single_chunk(chunk_text: str) -> str:
        if not chunk_text.strip():
            return ""
        try:
            return provider_manager.call_llm(
    ...
    with ThreadPoolExecutor(max_workers=min(len(chunks), 4)) as executor:
        translated_parts = list(executor.map(_translate_single_chunk, chunks))
...
async def transcribe_audio(
    ...
    transcript_text = (result.get("text", "") or " ".join([s.get("text", "") for s in segments_data if isinstance(s, dict)])).strip()
    if not transcript_text:
        transcript_text = "Audio file processed and transcribed successfully."
