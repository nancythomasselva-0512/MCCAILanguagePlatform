      ...
      <div className="max-w-7xl mx-auto w-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 w-full">
          {displayPlans.map((plan: any, idx: number) => {
            ...
            const audioMins = plan.voice_minutes_limit ?? plan.audio_minutes_limit ?? plan.transcription_limit ?? (isFree ? 15 : 60);
            ...
            const storageMb = plan.storage_limit || 50;
                    ...
                    <div className="flex items-center justify-between gap-2 mb-1.5">
                      ...
                      {plan.trial_days !== undefined && plan.trial_days !== null && (
                        <span className="text-[10px] font-extrabold uppercase tracking-wider px-2.5 py-0.5 rounded-full bg-teal-500/10 text-teal-600 dark:text-teal-400 border border-teal-500/20">
                          {plan.trial_days === 0 ? 'Lifetime Access' : `${plan.trial_days} Days Free`}
                        </span>
                      )}
                    </div>
                              ...
                              <div key={catTitle} className="space-y-1.5">
                                ...
                                <div className="flex flex-wrap gap-1.5 pt-0.5">
                                  {group.items.map((label: string, fIdx: number) => (
                                    <span
                                      key={fIdx}
                                      className={`text-[10px] sm:text-[11px] font-bold px-2.5 py-1 rounded-lg inline-flex items-center gap-1 shadow-2xs ${cardStyles[idx % 4].chip}`}
                                    >
                                      ✓ {label}
                                    </span>
                                  ))}
