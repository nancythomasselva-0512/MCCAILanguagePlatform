...
const inputCls =
  "w-full mt-1 px-3 py-2.5 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-xl outline-none text-slate-900 dark:text-white font-medium focus:ring-2 focus:ring-indigo-500/40 transition-all placeholder:text-slate-400 dark:placeholder:text-slate-500 dark:[color-scheme:dark]";
    ...
    <>
      <div className="space-y-6 animate-fadeIn">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm font-bold text-slate-500 dark:text-slate-400 mt-1">
              Configure email delivery, templates, and notifications.
            </p>
          </div>
          <button
            onClick={handleSave}
        ...
        <div className="flex gap-4 border-b border-slate-200 dark:border-white/10 pb-4">
          {(["config", "templates", "logs"] as const).map((tab) => (
            <button
              key={tab}
            ...
            >
              {tab === "config" ? "SMTP Configuration" : tab === "templates" ? "Email Templates" : "Email Logs"}
            </button>
          ))}
        </div>
          ...
          <div className="space-y-6">
            <div className="glass-card rounded-2xl p-6 border border-slate-200 dark:border-white/5 bg-white dark:bg-[#111827]/40">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-lg font-black text-slate-900 dark:text-white">SMTP Server Details</h3>
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                      config.is_enabled ? "bg-teal-500" : "bg-slate-300 dark:bg-slate-700"
                    }`}
                  >
                    <span
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="flex items-center mt-3">
                    <button
                      onClick={() =>
                        setConfig({ ...config, enable_authentication: !config.enable_authentication })
                      }
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                        config.enable_authentication ? "bg-indigo-500" : "bg-slate-300 dark:bg-slate-700"
                      }`}
                    >
                      <span
                        className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                          config.enable_authentication ? "translate-x-6" : "translate-x-1"
                        }`}
                      />
                    </button>
                    ...
                    <div>
                      ...
                      <div className="relative mt-1">
                        <input
                          ...
                          className="w-full px-3 py-2.5 pr-10 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-xl outline-none text-slate-900 dark:text-white font-medium focus:ring-2 focus:ring-indigo-500/40 transition-all placeholder:text-slate-400 dark:placeholder:text-slate-500"
                        />
                        <button
                          type="button"
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. noreply@fluentia.com"
                    className={inputCls}
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setConfig({ ...config, connection_timeout: Number(e.target.value) })}
                    className={inputCls}
                  />
                </div>
              </div>
            ...
            {/* Test Email */}
            <div className="glass-card rounded-2xl p-6 border border-slate-200 dark:border-white/5 bg-white dark:bg-[#111827]/40">
              <h3 className="text-lg font-black mb-4 text-slate-900 dark:text-white">Send Test Email</h3>
              <div className="flex gap-4 items-center">
                <input
                  ...
                  className="flex-1 max-w-md px-3 py-2.5 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-xl outline-none text-slate-900 dark:text-white font-medium focus:ring-2 focus:ring-indigo-500/40 transition-all placeholder:text-slate-400 dark:placeholder:text-slate-500"
                />
                <button
          ...
          <div className="glass-card rounded-2xl p-6 border border-slate-200 dark:border-white/5 bg-white dark:bg-[#111827]/40">
            ...
            <div className="space-y-4">
              {templates.map((tpl) => (
                <div
                  key={tpl.id}
                  className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-white/5"
                >
                  <div>
                    <h4 className="font-bold text-sm text-slate-900 dark:text-white capitalize">
                      {tpl.template_type.replace("_", " ")}
                    </h4>
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Subject: {tpl.subject}</p>
                  </div>
                  <div className="flex gap-3 items-center">
                    <button
                      className={`relative inline-flex h-6 w-11 items-center rounded-full cursor-default ${
                        tpl.is_enabled ? "bg-teal-500" : "bg-slate-300 dark:bg-slate-700"
                      }`}
                    >
                      <span
                        className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                          tpl.is_enabled ? "translate-x-6" : "translate-x-1"
                        }`}
                      />
                    </button>
                    <button
              ...
              <tbody>
                {logs.length > 0 ? (
                  logs.map((log) => (
                    <tr
                      key={log.id}
                      className="border-b border-slate-200 dark:border-white/5 last:border-0 text-slate-900 dark:text-white"
                    >
