...
export const TenantBilling: React.FC = () => {
  ...
  return (
    <div className="space-y-6 w-full p-4 md:p-8 animate-fadeIn">
      {/* Header Banner */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-white/5 pb-5">
        <div>
          ...
          <p className="text-base text-slate-650 dark:text-slate-400 font-semibold mt-1">
          ...
          <div className="lg:col-span-2 app-card rounded-2xl p-6 space-y-6 flex flex-col justify-between">
            <div className="space-y-4">
              <div className="flex justify-between items-start">
                <div>
                  <span className="text-sm font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Current Plan Tier</span>
                ...
                <div className="text-right">
                  <span className="text-sm font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Recurring Price</span>
                  <p className="text-2xl font-black text-teal-600 dark:text-teal-400 mt-0.5">{getCurrencySymbol(data?.currency || 'INR')}{activePlan.price} <span className="text-base font-medium text-slate-500 dark:text-slate-400">/ mo</span></p>
                </div>
              </div>

              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 pt-4 border-t border-slate-200 dark:border-white/5 text-base text-slate-500 dark:text-slate-400">
                <div>
                  <span className="block text-sm uppercase font-bold text-slate-450 dark:text-slate-500">Billing Cycle</span>
                  <strong className="text-slate-900 dark:text-white capitalize mt-0.5 block">{activePlan.billing_cycle}</strong>
                </div>
                <div>
                  <span className="block text-sm uppercase font-bold text-slate-450 dark:text-slate-500">Expiry Date</span>
                  <strong className="text-slate-900 dark:text-white mt-0.5 block">{activePlan.expiry_date || "Lifetime"}</strong>
                </div>
                <div className="col-span-2 md:col-span-1">
                  <span className="block text-sm uppercase font-bold text-slate-450 dark:text-slate-500">Support Level</span>
            ...
            {activePlan.name !== 'Free' && (
              <div className="flex gap-3 pt-6 border-t border-slate-200 dark:border-white/5 justify-end">
                <button
                  ...
                  className="bg-red-500/10 hover:bg-red-500/20 text-red-600 dark:text-red-400 border border-red-500/10 px-4 py-2 rounded-xl text-base font-bold transition-all cursor-pointer"
                >
                  Cancel Plan
                </button>
                <button
                  onClick={handleRenewSubscription}
                  className="bg-teal-600 hover:bg-teal-700 text-white px-5 py-2 rounded-xl text-base font-bold transition-all cursor-pointer"
                >
                  Renew Plan
                </button>
              </div>
          ...
          <div className="app-card rounded-2xl p-6 flex flex-col justify-between space-y-4">
            <div>
              <h3 className="text-sm font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-1.5">
                <TrendingUp size={16} className="text-teal-500 dark:text-teal-400" />
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="w-full h-1.5 bg-slate-200 dark:bg-white/5 rounded-full overflow-hidden">
                      <div className="h-full bg-teal-500" style={{ width: `${Math.min(100, (usage.audio_minutes_used / usage.audio_minutes_limit) * 100)}%` }} />
                    </div>
                  </div>
            ...
            <div className="text-sm text-slate-600 dark:text-slate-500 leading-relaxed font-semibold flex items-start gap-1">
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-sm text-slate-550 dark:text-slate-500 font-semibold mt-0.5">Select a target tier to upgrade or downgrade your active workspace instantly.</p>
        </div>
            ...
            return (
              <div 
                ...
                className={`p-5 rounded-2xl border flex flex-col justify-between transition-all relative overflow-hidden group ${
                  isCurrent 
                    ? 'border-teal-500 bg-teal-50/50 dark:bg-teal-950/10 shadow-lg shadow-teal-100 dark:shadow-teal-950/20' 
              ...
              >
                {isCurrent && (
                  <div className="absolute top-0 right-0 bg-teal-600 text-white text-sm font-black uppercase tracking-widest px-3 py-1 rounded-bl-xl">
                    Active
                  </div>
                )}
                ...
                <div>
                  ...
                  <ul className="mt-5 space-y-2.5 text-sm font-semibold text-slate-600 dark:text-slate-400 border-t border-slate-200 dark:border-white/5 pt-4">
                ...
                <div className="mt-6 pt-4 border-t border-slate-200 dark:border-white/5">
                  {isCurrent ? (
                    <button 
                      disabled
                      className="w-full py-2 rounded-xl bg-white dark:bg-white/5 text-slate-500 dark:text-slate-400 text-base font-bold border border-slate-200 dark:border-white/5 text-center cursor-default"
                    >
                      Active Plan
                    </button>
                  ) : (
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 py-2 rounded-xl bg-teal-600 hover:bg-teal-700 text-white text-base font-bold text-center cursor-pointer transition-all"
                      >
                        Subscribe
                      </button>
                    </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-base border-collapse">
              ...
              <tbody>
                    ...
                    <td className="py-3 text-right">
                      {inv.status === 'pending' ? (
                        <button
                          ...
                          className="bg-emerald-600 hover:bg-emerald-700 text-white px-2.5 py-1 rounded text-sm font-bold cursor-pointer"
                        >
                          Pay Now
                        </button>
                      ) : (
                        inv.pdf_path && (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="bg-white dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-650 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white px-2.5 py-1 rounded text-sm font-bold border border-slate-200 dark:border-white/5 cursor-pointer inline-flex items-center gap-1 transition-all"
                          >
                ...
                {invoices.length === 0 && (
                  <tr>
                    <td colSpan={7} className="text-center py-10 text-base text-slate-500 dark:text-slate-400 font-semibold">
                      No invoices found.
                    </td>
                  </tr>
                )}
            ...
            <table className="w-full text-left text-base border-collapse">
              ...
              <tbody>
                ...
                {(data?.subscription_history || []).length === 0 && (
                  <tr>
                    <td colSpan={6} className="text-center py-10 text-base text-slate-500 dark:text-slate-400 font-semibold">
          ...
          <div className="relative w-full max-w-xl overflow-hidden rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 shadow-2xl p-6 space-y-6 flex flex-col justify-between">
            ...
            <div className="flex justify-between items-start border-b border-slate-200 dark:border-white/5 pb-4">
              <div>
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 font-semibold mt-1">Completing payment securely via automated gateway integration.</p>
              </div>
              <button 
            ...
            {processingPayment && (
              <div className="flex flex-col items-center justify-center py-12 space-y-4">
                ...
                <p className="text-base text-slate-500 dark:text-slate-400 font-semibold">Verifying secure checkout session with payment nodes...</p>
              </div>
            )}
            ...
            {!processingPayment && paymentResult === 'success' && completedSessionData && (
              <div className="flex flex-col items-center justify-center py-6 space-y-4 animate-fadeIn">
                ...
                <p className="text-base text-slate-600 dark:text-slate-400 text-center max-w-sm">
                  Your workspace has been upgraded to the <strong>{completedSessionData.planName}</strong> plan.
                </p>
                
                {/* Transaction details box */}
                <div className="w-full bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-white/5 rounded-xl p-4 space-y-2.5 text-base text-slate-600 dark:text-slate-400">
                  ...
                  <div className="flex justify-between">
                    <span>Status:</span>
                    <span className="px-2 py-0.5 rounded text-sm font-bold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
                      PAID
                    </span>
                  </div>
                </div>
                ...
                <div className="w-full pt-2">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-center justify-center gap-1.5 w-full bg-teal-50 dark:bg-teal-600/10 hover:bg-teal-100 dark:hover:bg-teal-600/20 text-teal-650 dark:text-teal-400 border border-teal-200 dark:border-teal-500/10 px-4 py-2.5 rounded-xl text-base font-bold transition-all cursor-pointer"
                  >
                ...
                <button
                  onClick={() => setCheckoutModalOpen(false)}
                  className="w-full bg-teal-600 hover:bg-teal-700 text-white py-2.5 rounded-xl text-base font-bold transition-all cursor-pointer mt-4"
                >
                  Done
                </button>
              </div>
            ...
            {!processingPayment && paymentResult === 'success' && !completedSessionData && (
              <div className="flex flex-col items-center justify-center py-6 space-y-4 animate-fadeIn">
                <Loader2 className="animate-spin text-teal-500" size={32} />
                <p className="text-base text-slate-500 dark:text-slate-400 font-semibold">Updating billing state...</p>
              </div>
            )}
              ...
              <div className="space-y-4 animate-fadeIn">
                <div className="bg-slate-50 dark:bg-slate-950/30 border border-slate-200 dark:border-white/5 rounded-2xl p-5 space-y-4">
                  <div className="flex justify-between items-center pb-3 border-b border-slate-200 dark:border-white/5">
                    <div>
                      <span className="text-sm font-bold text-slate-500 uppercase">Selected Subscription Plan</span>
                      <h4 className="text-lg font-black text-slate-900 dark:text-white capitalize">{selectedPlan.name} Plan</h4>
                    </div>
                    <span className="px-3 py-1 rounded-full text-sm font-bold bg-teal-50 dark:bg-teal-600/10 text-teal-600 dark:text-teal-400 border border-teal-200 dark:border-teal-500/20 capitalize">
                      {billingCycle} billing
                    </span>
                  </div>

                  <div className="space-y-2.5 text-base text-slate-650 dark:text-slate-400">
                    <div className="flex justify-between">
                ...
                <div className="border-t border-slate-200 dark:border-white/5 pt-4 flex justify-between gap-3">
                  <button
                    onClick={() => setCheckoutModalOpen(false)}
                    className="bg-white dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-white/5 px-4 py-2.5 rounded-xl text-base font-bold transition-all cursor-pointer flex-1"
                  >
                    Cancel
                  </button>
                  <button
                    onClick={() => setCheckoutStep('payment')}
                    className="bg-teal-600 hover:bg-teal-700 text-white px-5 py-2.5 rounded-xl text-base font-bold transition-all cursor-pointer flex-1"
                  >
                    Proceed to Payment
                  </button>
                </div>
              ...
              <div className="space-y-6 animate-fadeIn">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-2">
                    <span className="text-sm font-black uppercase text-slate-500 tracking-wider">Gateway method</span>
                    
                    {checkoutSession.gateways.stripe.enabled && (
                      <button
                        onClick={() => setActiveGateway('stripe')}
                        className={`w-full flex items-center gap-2 p-3 rounded-xl border text-base font-bold transition-all cursor-pointer ${
                  ...
                  <div className="md:col-span-2 space-y-4">
                    <div className="p-4 rounded-xl bg-slate-55 dark:bg-slate-950/30 border border-slate-200 dark:border-white/5">
                      <span className="text-sm font-black uppercase text-slate-500 tracking-wider">Plan Billing Details</span>
                    ...
                    {errorMsg && (
                      <div className="p-3 rounded-xl bg-red-50/10 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 text-red-650 dark:text-red-500 text-base font-semibold flex items-start gap-1.5 leading-relaxed">
                    ...
                    {activeGateway === 'stripe' && (
                      <div className="space-y-3 animate-fadeIn">
                        <div className="space-y-1">
                          <label className="text-sm font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Card Number</label>
                          <input
                            type="text"
                            value={cardNumber}
                            onChange={(e) => setCardNumber(e.target.value)}
                            placeholder="4242 4242 4242 4242"
                            className="w-full px-3 py-2 rounded-xl text-base bg-white dark:bg-slate-950/40 border border-slate-300 dark:border-white/5 text-slate-900 dark:text-white outline-none"
                          />
                        </div>
                        <div className="grid grid-cols-2 gap-3">
                          <div className="space-y-1">
                            <label className="text-sm font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Expiration</label>
                            <input
                              type="text"
                              value={cardExpiry}
                          ...
                          <div className="space-y-1">
                            <label className="text-sm font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">CVC</label>
                            <input
                              type="password"
                              value={cardCvc}
                    ...
                    {activeGateway === 'razorpay' && (
                      <div className="space-y-3 animate-fadeIn">
                        <div className="space-y-1">
                          <label className="text-sm font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Preferred Bank</label>
                          <select
                            value={razorpayBank}
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-450 leading-relaxed font-semibold">Razorpay will redirect to secure net banking site to verify credentials after validation.</p>
                      </div>
                    )}
                    ...
                    {activeGateway === 'upi' && (
                      <div className="space-y-3 text-center flex flex-col items-center justify-center animate-fadeIn">
                        ...
                        <div className="space-y-1 w-full text-left mt-2">
                          <label className="text-sm font-bold uppercase tracking-wider text-slate-550 dark:text-slate-400">Scan & Pay QR or Enter UPI Address</label>
                          <input
                            type="text"
                            value={upiAddress}
                ...
                <div className="border-t border-slate-200 dark:border-white/5 pt-4 flex justify-between gap-3">
                  ...
                  <button
                    ...
                    className="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-xl text-base font-bold transition-all cursor-pointer flex-1"
                  >
                    Pay Now
                  </button>
                </div>
