...
import { 
  CreditCard, Check, AlertTriangle, Loader2, Sparkles, Server, Clock, 
  RefreshCw, Ban, CheckCircle2, Copy, TrendingUp, Layers, Settings, 
  Eye, Info, Download, QrCode, ShieldCheck, HelpCircle, ArrowUpRight
} from 'lucide-react';
    ...
    <div className="space-y-6 max-w-7xl mx-auto p-4 md:p-8 animate-fadeIn">
      ...
      {activePlan && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 app-card rounded-2xl p-6 border border-white/5 bg-slate-900/30 space-y-6 flex flex-col justify-between">
            <div className="space-y-4">
              <div className="flex justify-between items-start">
                <div>
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Current Plan Tier</span>
                ...
                <div className="text-right">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Recurring Price</span>
                  <p className="text-2xl font-black text-blue-400 mt-0.5">${activePlan.price} <span className="text-xs font-medium text-slate-400">/ mo</span></p>
                </div>
              </div>

              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 pt-4 border-t border-white/5 text-xs text-slate-400">
                <div>
                  <span className="block text-[10px] uppercase font-bold text-slate-500">Billing Cycle</span>
                  <strong className="text-white capitalize mt-0.5 block">{activePlan.billing_cycle}</strong>
                </div>
                <div>
                  <span className="block text-[10px] uppercase font-bold text-slate-500">Expiry Date</span>
                  <strong className="text-white mt-0.5 block">{activePlan.expiry_date || "Lifetime"}</strong>
                </div>
                <div className="col-span-2 md:col-span-1">
                  <span className="block text-[10px] uppercase font-bold text-slate-500">Support Level</span>
            ...
            {activePlan.name !== 'Free' && (
              <div className="flex gap-3 pt-6 border-t border-white/5 justify-end">
                <button
                  onClick={handleCancelSubscription}
                  className="bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/10 px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer"
                >
                  Cancel Plan
                </button>
                <button
          ...
          {/* Quick Metrics */}
          <div className="app-card rounded-2xl p-6 border border-white/5 bg-slate-900/30 flex flex-col justify-between space-y-4">
            <div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-xs">
                      <span className="text-slate-400">Transcription Limit</span>
                      <strong className="text-white">{usage.audio_minutes_used.toFixed(1)} / {usage.audio_minutes_limit} mins</strong>
                    </div>
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-xs">
                      <span className="text-slate-400">Translation Limit</span>
                      <strong className="text-white">{(usage.translation_chars_used).toLocaleString()} / {(usage.translation_chars_limit).toLocaleString()} Chars</strong>
                    </div>
                    <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden">
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-xs">
                      <span className="text-slate-400">TTS Audio Limit</span>
                      <strong className="text-white">{(usage.tts_chars_used).toLocaleString()} / {(usage.tts_chars_limit).toLocaleString()} Chars</strong>
                    </div>
                    <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden">
            ...
            <div className="text-[10px] text-slate-500 leading-relaxed font-semibold flex items-start gap-1">
      ...
      <div className="space-y-4">
        <div>
          <h3 className="text-sm font-bold text-white">Compare Subscriptions & Plans</h3>
          <p className="text-[10px] text-slate-500 font-semibold mt-0.5">Select a target tier to upgrade or downgrade your active workspace instantly.</p>
        </div>
            ...
            return (
              <div 
                ...
                className={`p-5 rounded-2xl border flex flex-col justify-between transition-all relative overflow-hidden group ${
                  isCurrent 
                    ? 'border-blue-500 bg-blue-950/10 shadow-lg shadow-blue-950/20' 
              ...
              >
                ...
                <div>
                  <h4 className="text-base font-extrabold text-white capitalize">{p.name}</h4>
                  <div className="mt-3 text-2xl font-black text-white">
                    ${p.price}
                    <span className="text-[10px] font-bold text-slate-500 ml-1">/ month</span>
                  </div>

                  <ul className="mt-5 space-y-2.5 text-[10px] font-semibold text-slate-400 border-t border-white/5 pt-4">
                    <li className="flex justify-between items-center">
                      <span>Audio Transcriptions:</span>
                      <strong className="text-white">{p.transcription_limit} mins</strong>
                    </li>
                    <li className="flex justify-between items-center">
                      <span>Translation characters:</span>
                      <strong className="text-white">{(p.translation_limit || 0).toLocaleString()}</strong>
                    </li>
                    <li className="flex justify-between items-center">
                      <span>TTS Voice Synthesis:</span>
                      <strong className="text-white">{(p.tts_limit || 0).toLocaleString()}</strong>
                    </li>
                    <li className="flex justify-between items-center">
                      <span>Cloud Storage:</span>
                      <strong className="text-white">{p.storage_limit} MB</strong>
                    </li>
                  </ul>
                </div>
                ...
                <div className="mt-6 pt-4 border-t border-white/5">
                  {isCurrent ? (
                    <button 
                      disabled
                      className="w-full py-2 rounded-xl bg-white/5 text-slate-400 text-xs font-bold border border-white/5 text-center cursor-default"
                    >
                      Active Plan
                    </button>
                  ) : (
      ...
      <div className="app-card rounded-2xl p-6 border border-white/5 bg-slate-900/30 space-y-6">
        {/* Navigation Tabs */}
              ...
              <tbody>
                {invoices.map((inv: any) => (
                  <tr key={inv.id} className="border-b border-white/5 hover:bg-white/5 transition-all">
                    ...
                    <td className="py-3 text-slate-350">{inv.plan_name}</td>
                    <td className="py-3 text-white font-bold">${inv.total_amount.toFixed(2)}</td>
                    <td className="py-3 text-slate-400 font-mono">{new Date(inv.due_date).toLocaleDateString()}</td>
                      ...
                      {inv.status === 'pending' ? (
                        ...
                        inv.pdf_path && (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="bg-white/5 hover:bg-white/10 text-slate-300 hover:text-white px-2.5 py-1 rounded text-[10px] font-bold cursor-pointer inline-flex items-center gap-1 transition-all"
                          >
              ...
              <tbody>
                {(data?.payments || []).map((pay: any) => (
                  <tr key={pay.id} className="border-b border-white/5 hover:bg-white/5 transition-all">
                    <td className="py-3 font-mono font-bold text-white">{pay.transaction_id || `TXN-${pay.id.slice(0, 8)}`}</td>
                    <td className="py-3 font-mono text-slate-350">{pay.invoice_number}</td>
                    <td className="py-3 text-slate-400 capitalize">{pay.payment_method}</td>
                    <td className="py-3 text-white font-bold">${pay.amount.toFixed(2)}</td>
                    <td className="py-3 text-slate-400 font-mono">{new Date(pay.created_at).toLocaleString()}</td>
                    <td className="py-3">
                      <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase ${
            ...
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody>
                {(data?.subscription_history || []).map((sh: any) => (
                  <tr key={sh.id} className="border-b border-white/5 hover:bg-white/5 transition-all">
                    <td className="py-3 font-bold text-white capitalize">{sh.plan_name}</td>
                    <td className="py-3">
                      <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase ${
                        sh.action === 'Upgrade' ? 'bg-blue-500/10 text-blue-400 border border-blue-500/20' : 
                        sh.action === 'Renew' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' :
                        'bg-red-500/10 text-red-500 border border-red-500/20'
                      }`}>
                        {sh.action}
                      </span>
                    </td>
                    <td className="py-3 text-white font-bold">${sh.price.toFixed(2)}</td>
                    <td className="py-3 text-slate-400 font-mono">{sh.start_date}</td>
                    <td className="py-3 text-slate-400 font-mono">{sh.end_date}</td>
                    <td className="py-3 text-slate-450 font-mono">{new Date(sh.created_at).toLocaleDateString()}</td>
                  </tr>
                ))}
                {(data?.subscription_history || []).length === 0 && (
                  <tr>
                    <td colSpan={6} className="text-center py-10 text-xs text-slate-400 font-semibold">
      ...
      {checkoutModalOpen && selectedPlan && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fadeIn">
          <div className="relative w-full max-w-xl overflow-hidden rounded-2xl bg-slate-900 border border-white/10 shadow-2xl p-6 space-y-6 flex flex-col justify-between">
            ...
            <div className="flex justify-between items-start border-b border-white/5 pb-4">
              <div>
                ...
                <p className="text-[10px] text-slate-400 font-semibold mt-1">Completing payment securely via automated gateway integration.</p>
              </div>
              <button 
            ...
            {processingPayment && (
              <div className="flex flex-col items-center justify-center py-12 space-y-4">
                ...
                <p className="text-xs text-slate-400 font-semibold">Verifying secure checkout session with payment nodes...</p>
              </div>
            )}
            ...
            {!processingPayment && paymentResult === 'success' && completedSessionData && (
              <div className="flex flex-col items-center justify-center py-6 space-y-4 animate-fadeIn">
                <div className="h-16 w-16 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 flex items-center justify-center">
                ...
                <div className="w-full bg-slate-950/40 border border-white/5 rounded-xl p-4 space-y-2.5 text-xs text-slate-400">
                  <div className="flex justify-between">
                    <span>Invoice Number:</span>
                    <strong className="text-white font-mono">{completedSessionData.invoiceNumber}</strong>
                  </div>
                  <div className="flex justify-between">
                    <span>Transaction ID:</span>
                    <strong className="text-white font-mono">{completedSessionData.transactionId}</strong>
                  </div>
                  <div className="flex justify-between">
                    <span>Activated Plan:</span>
                    <strong className="text-white capitalize">{completedSessionData.planName}</strong>
                  </div>
                  <div className="flex justify-between">
                ...
                <div className="grid grid-cols-2 gap-3 w-full pt-2">
                  <a
                    href={`http://127.0.0.1:8000/api/billing/invoices/${completedSessionData.invoiceId}/download`}
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-center justify-center gap-1.5 bg-white/5 hover:bg-white/10 text-slate-355 hover:text-white border border-white/5 px-4 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer"
                  >
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-center justify-center gap-1.5 bg-blue-600/10 hover:bg-blue-600/20 text-blue-400 border border-blue-500/10 px-4 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer"
                  >
            ...
            {!processingPayment && paymentResult !== 'success' && checkoutSession && checkoutStep === 'confirm' && (
              <div className="space-y-4 animate-fadeIn">
                <div className="bg-slate-950/30 border border-white/5 rounded-2xl p-5 space-y-4">
                  <div className="flex justify-between items-center pb-3 border-b border-white/5">
                    <div>
                      <span className="text-[10px] font-bold text-slate-500 uppercase">Selected Subscription Plan</span>
                      <h4 className="text-lg font-black text-white capitalize">{selectedPlan.name} Plan</h4>
                    </div>
                    <span className="px-3 py-1 rounded-full text-[10px] font-bold bg-blue-600/10 text-blue-400 border border-blue-500/20 capitalize">
                  ...
                  <div className="space-y-2.5 text-xs text-slate-400">
                    <div className="flex justify-between">
                      ...
                      <strong className="text-white">${checkoutSession.base_amount.toFixed(2)}</strong>
                    </div>
                    <div className="flex justify-between">
                      <span>GST Amount (18%):</span>
                      <strong className="text-white">${checkoutSession.tax_amount.toFixed(2)}</strong>
                    </div>
                    <div className="flex justify-between pt-2.5 border-t border-white/5 text-sm">
                      <span className="font-bold text-slate-300">Total Amount Due:</span>
                      <strong className="text-emerald-400 font-black">${checkoutSession.amount.toFixed(2)} {checkoutSession.currency}</strong>
                    </div>
                  </div>
                </div>

                <div className="border-t border-white/5 pt-4 flex justify-between gap-3">
                  <button
                    onClick={() => setCheckoutModalOpen(false)}
                    className="bg-white/5 hover:bg-white/10 text-slate-300 border border-white/5 px-4 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer flex-1"
                  >
                    Cancel
                  </button>
                  <button
              ...
              <div className="space-y-6 animate-fadeIn">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-2">
                      ...
                      <button
                        ...
                        className={`w-full flex items-center gap-2 p-3 rounded-xl border text-xs font-bold transition-all cursor-pointer ${
                          activeGateway === 'stripe' 
                            ? 'border-blue-500 bg-blue-950/15 text-white' 
                            : 'border-white/5 bg-slate-950/20 text-slate-400 hover:text-white'
                        }`}
                      >
                        <span className="text-sm">💳</span> Stripe Test
                      </button>
                    )}
                    ...
                    {checkoutSession.gateways.razorpay.enabled && (
                      ...
                      >
                        <span className="text-sm">💳</span> Razorpay Test
                      </button>
                    )}
                    ...
                    {checkoutSession.gateways.upi.enabled && (
                      ...
                      >
                        <span className="text-sm">📱</span> UPI Test
                      </button>
                    )}
                  </div>
                  ...
                  <div className="md:col-span-2 space-y-4">
                    <div className="p-4 rounded-xl bg-slate-950/30 border border-white/5">
                      ...
                      <div className="flex justify-between items-center mt-2 text-xs font-semibold">
                        <span className="text-slate-400">{selectedPlan.name} Plan ({billingCycle})</span>
                        <strong className="text-white">${checkoutSession.amount.toFixed(2)} {checkoutSession.currency}</strong>
                      </div>
                    </div>

                    {errorMsg && (
                      <div className="p-3 rounded-xl bg-red-500/10 border border-red-500/20 text-red-500 text-xs font-semibold flex items-start gap-1.5 leading-relaxed">
                    ...
                    {activeGateway === 'stripe' && (
                      <div className="space-y-3 animate-fadeIn">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          <div className="space-y-1">
                            <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400">CVC</label>
                            <input
                              type="password"
                              value={cardCvc}
                    ...
                    {activeGateway === 'razorpay' && (
                      <div className="space-y-3 animate-fadeIn">
                        <div className="space-y-1">
                          <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Preferred Bank</label>
                          <select
                            value={razorpayBank}
                        ...
                        <p className="text-[10px] text-slate-500 leading-relaxed font-semibold">Razorpay will redirect to secure net banking site to verify credentials after validation.</p>
                      </div>
                    )}
                    ...
                    {activeGateway === 'upi' && (
                      <div className="space-y-3 text-center flex flex-col items-center justify-center animate-fadeIn">
                        ...
                        <div className="space-y-1 w-full text-left mt-2">
                          <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Scan & Pay QR or Enter UPI Address</label>
                          <input
                            type="text"
                            value={upiAddress}
                ...
                <div className="border-t border-white/5 pt-4 flex justify-between gap-3">
                  ...
                  <button
                    ...
                    className="bg-red-900/40 hover:bg-red-900/60 text-red-200 border border-red-900/20 px-4 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer flex-1"
                  >
                    Fail Payment
                  </button>
                  <button
